...
This script:
  ...
  - Renders into a Word template (e.g., COP24 Q4 DATIM File Narrative Final.docx)

Usage:
  python generate_cdc_report_consistency.py \
    --consistency Consistency_Report_COP2024_Q4.xlsx \
    --template "COP24 Q4 DATIM File Narrative Final.docx" \
...
def load_consistency(path: str) -> pd.DataFrame:
    ...
    # Expect something like 'Indicator' and 'Number of Reporting Units'
    ind_col = None
    num_col = None
    for c in df.columns:
        ...
        if 'indicator' in cname:
            ind_col = c
        if 'number' in cname and 'report' in cname:
            num_col = c
...
def main():
    ...
    parser.add_argument('--template', required=True, help='Path to CDC report template DOCX')
